...
{
    /// <summary>
    /// A variant of MCTS that uses the Sliding Window UCB policy as its tree policy. It differs from SWUCBMCTS
    /// in that it also uses its windowed estimates when picking moves at the end of its run.
    /// </summary>
    internal class SWUCBMCTS2 : BasicMCTS
    {
        /// <summary>
        ...
        {
            if (specification.StartsWith("swucbmcts2_"))
            {
                ...
                if (parts.Length == 4)
                {
                    ...
                    return new SWUCBMCTS2(window, longExplore, c);
                }
                else
                {
                    return new SWUCBMCTS2(window, longExplore);
                }
            }
        ...
        public SWUCBMCTS2(int window, bool longExplore)
        {
            WRTMCTSNode2.windowSize = window;
            this.longExplore = longExplore;
        }

        public SWUCBMCTS2(int window, bool longExplore, double c) : base(c)
        {
            WRTMCTSNode2.windowSize = window;
            this.longExplore = longExplore;
        }
        ...
        {
            root = new WRTMCTSNode2(initialState, initialState.CurrentPlayer, null);
            WRTMCTSNode2.initialTeam = initialState.CurrentPlayer;
        }
        ...
        {
            double rewardsSum = ((WRTMCTSNode2)n).GetRewards().Sum();

            if (longExplore)
            {
                return rewardsSum / Math.Min(WRTMCTSNode2.windowSize, n.Visits) + c * Math.Sqrt(Math.Log(n.parent.Visits) / n.Visits);
            }
            else
            {
